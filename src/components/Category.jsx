...
function Category() {
  return (
    ...
    <div className="flex justify-between m-1 p-1 border-b border-gray-200 max-h-24">
      {categoryList.map(category => (
        <div id = {category.id} className ="text-sm mg-6 p-6">{category.category}</div>
      ))}
      <button className="m-2 p-2 border-b border-gray-200 max-h-24">
      &gt;
   </button>
   <button className="m-2 p-2 border-b border-gray-200 max-h-24">
      필터
